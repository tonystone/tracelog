...
public class FileWriter: OutputStreamWriter {
    ...
    public func write(_ entry: Writer.LogEntry) -> Result<Int, FailureReason> {
        ...
        if self.file.stream.position + UInt64(bytes.count) >= file.config.maxSize {
            self.file = rotate(file: self.file, fallbackStream: FileOutputStream(fileDescriptor: STDOUT_FILENO, closeFd: false, url: URL(fileURLWithPath: "//dev/null")), dateFormatter: self.fileNameDateFormatter)
        }
